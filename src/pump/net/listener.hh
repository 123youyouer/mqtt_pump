...
namespace reactor{
    ...
    struct schedule_to<reactor::task_schedule_center<net::connection_data>>{
        static void apply(reactor::task_schedule_center<net::connection_data>* dst,utils::noncopyable_function<void(net::connection_data&&)>&& f){
            dst->push(std::forward<utils::noncopyable_function<void(net::connection_data&&)>>(f));
        }
    };
    template <typename _T0>
    struct schedule_to<_T0,reactor::task_schedule_center<net::connection_data>>{
        static void apply(reactor::task_schedule_center<net::connection_data>* dst,utils::noncopyable_function<void(net::connection_data&&)>&& f){
            schedule_to<reactor::task_schedule_center<net::connection_data>>::apply(dst,std::forward<utils::noncopyable_function<void(net::connection_data&&)>>(f));
        }
    };
}
