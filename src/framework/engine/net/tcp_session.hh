...
namespace engine{
    namespace net{
        ...
        struct tcp_session:boost::noncopyable{
            using recv_sch_args_t=FLOW_ARG(common::linear_ringbuffer_st*);
            using send_sch_args_t=FLOW_ARG(send_proxy);
            struct recv_handler{
                size_t need_len;
                common::ncpy_func<bool()> available;
                common::ncpy_func<void(FLOW_ARG(common::linear_ringbuffer_st*)&&)> handle;
            };
            struct data{
                ...
                common::linear_ringbuffer_st _recv_buf;
            ...
            ALWAYS_INLINE auto
            wait_packet_with_timeout(int ms,size_t len=0){
                if(_data->_recv_buf.empty() || _data->_recv_buf.size()<len){
                    using f_type=common::ncpy_func<void(FLOW_ARG(std::variant<int,common::linear_ringbuffer_st*>))>;
                    return er::flow_builder<std::variant<int,common::linear_ringbuffer_st*>>::at_schedule
                            (
                                    [data=_data,ms,len](std::shared_ptr<er::flow_implent<std::variant<int,common::linear_ringbuffer_st*>>> sp_flow){
                                        data->schedule(tcp_session::recv_handler{
                                                len,
                                                [sp_flow](){ return !sp_flow->called();},
                                                [sp_flow](FLOW_ARG(common::linear_ringbuffer_st*)&& v){
                                                    switch (v.index()){
                                                        case 0:
                                                            sp_flow->trigge(FLOW_ARG(std::variant<int,common::linear_ringbuffer_st*>)(std::get<0>(v)));
                                                            return;
                                                        case 1:
                                                            sp_flow->trigge(FLOW_ARG(std::variant<int,common::linear_ringbuffer_st*>)(std::get<1>(v)));
                                                            return;
                                                        default:
                                                            sp_flow->trigge(FLOW_ARG(std::variant<int,common::linear_ringbuffer_st*>)(
                                                                    std::variant<int,common::linear_ringbuffer_st*>(std::get<2>(v))));
                                                            return;
                                                    }
                                                }
                                        });
                                        engine::timer::_sp_timer_set->add_timer(ms,[ms,sp_flow](){
                                            sp_flow->trigge(FLOW_ARG(std::variant<int,common::linear_ringbuffer_st*>)(
                                                    std::variant<int,common::linear_ringbuffer_st*>(ms)));
                                        });
                                    },
                    ...
                    return er::make_imme_flow()
                            .then([_data=this->_data](FLOW_ARG()&& a){
                                return std::variant<int,common::linear_ringbuffer_st*>(&_data->_recv_buf);
                            });
                }
            }
