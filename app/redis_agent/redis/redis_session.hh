...
namespace redis_agent::redis{
    struct command_res_handler{
        pump::common::ncpy_func<bool()> available;
        pump::common::ncpy_func<void(FLOW_ARG(std::unique_ptr<command::redis_reply>)&&)> handle;
    };
    class redis_session : boost::noncopyable{
        struct _inner_{
            ...
            explicit
            _inner_(pump::dpdk::net::tcp_session&& tcp_session):tcp(std::forward<pump::dpdk::net::tcp_session>(tcp_session)){}
        };
        std::shared_ptr<_inner_> inner_data;
        void
        decode_and_update_reply(pump::common::ringbuffer* buf){
            ...
            switch (resp::decode(rpl->result,(const char*)buf->read_head(),offset,buf->size())){
                case resp::decode_state::st_complete:
                    ...
                    if(inner_data->waiting_handlers.empty()){
                        inner_data->waiting_replies.emplace(std::forward<std::unique_ptr<command::redis_reply>>(rpl));
                    }
                    else{
    ...
    public:
        explicit
        redis_session(pump::dpdk::net::tcp_session&& tcp){
            inner_data=std::make_shared<_inner_>(std::forward<pump::dpdk::net::tcp_session>(tcp));
        }
    ...
    wait_reply(redis_session&& session){
        if(!session.inner_data->waiting_replies.empty()){
            auto res=pump::reactor::make_imme_flow(std::move(session.inner_data->waiting_replies.front()));
            session.inner_data->waiting_replies.pop();
            return res;
        }
        else{
            return pump::reactor::flow_builder<std::unique_ptr<command::redis_reply>>::at_schedule(
                    [session=std::forward<redis_session>(session)](std::shared_ptr<pump::reactor::flow_implent<std::unique_ptr<command::redis_reply>>> sp_flow)mutable{
    ...
    read_proc(redis_session&& session){
        session.inner_data->tcp.wait_packet()
                .then([session=std::forward<redis_session>(session)](FLOW_ARG(std::variant<int,pump::common::ringbuffer*>)&& v)mutable{
                    ____forward_flow_monostate_exception(v);
    ...
    connect_redis(int kqfd, const char* ip,int port,int timeout){
        return pump::dpdk::net::connect(kqfd,ip,port,timeout)
                .then([](FLOW_ARG(std::variant<int,pump::dpdk::net::tcp_session>)&& a){
                    ...
                    switch (rtn.index()){
                        case 0:
                            ...
                            throw std::logic_error("timeout");
                        default:
                            return redis_session(std::move(std::get<pump::dpdk::net::tcp_session>(rtn)));
                    }
                });
    }
