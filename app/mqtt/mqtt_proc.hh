    ...
    struct mqtt_proc{
        ...
        loop_session(mqtt_session<_TCP_IMPL_>&& session){
            wait_packet(std::forward<mqtt_session<_TCP_IMPL_>>(session))
                    .then([session=std::forward<mqtt_session<_TCP_IMPL_>>(session)](FLOW_ARG(std::tuple<u_int8_t,u_int32_t,pump::common::ringbuffer*>)&& v)mutable{
                        ____forward_flow_monostate_exception(v);
                        auto [cmd,remaining,others]=std::get<std::tuple<u_int8_t,u_int32_t,pump::common::ringbuffer*>>(v);
        ...
        static pump::reactor::flow_builder<>
        wait_connected(mqtt_session<_TCP_IMPL_>&& session){
            return wait_packet(std::forward<mqtt_session<_TCP_IMPL_>>(session))
                    .then([session=std::forward<mqtt_session<_TCP_IMPL_>>(session)](FLOW_ARG(std::tuple<u_int8_t,u_int32_t,pump::common::ringbuffer*>)&& v)mutable{
                        ____forward_flow_monostate_exception(v);
                        auto [cmd,remaining,payload]=std::get<std::tuple<u_int8_t,u_int32_t,pump::common::ringbuffer*>>(v);
                        switch (mqtt_command_type(cmd&0xF0)){
                            case mqtt_command_type::connect:
