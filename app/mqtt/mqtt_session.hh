...
namespace mqtt{
    ...
    wait_command(mqtt_session<_TCP_IMPL_>&& session){
        return session._inner_data_->inner_tcp.wait_packet(TIMEOUT_MS,1)
                .then([](FLOW_ARG(std::variant<int,pump::common::ringbuffer*>)&& v)mutable{
                    ____forward_flow_monostate_exception(v);
                    auto&& d=std::get<std::variant<int,pump::common::ringbuffer*>>(v);
                    ____throw_timeout(d);
                    return std::get<pump::common::ringbuffer*>(d)->pop_uint_8();
                });
    }

    template <typename _TCP_IMPL_>
    pump::reactor::flow_builder<u_int32_t>
    wait_remaining(mqtt_session<_TCP_IMPL_>&& session,u_int32_t last,u_int32_t multiplier){
        if(multiplier>128*128*128)
            ...
            return session._inner_data_->inner_tcp.wait_packet(TIMEOUT_MS,1)
                    .then([session=std::forward<mqtt_session<_TCP_IMPL_>>(session),last,multiplier](FLOW_ARG(std::variant<int,pump::common::ringbuffer*>)&& v)mutable{
                        ____forward_flow_monostate_exception(v);
                        auto&& d=std::get<std::variant<int,pump::common::ringbuffer*>>(v);
                        ____throw_timeout(d);
                        u_int32_t u=std::get<pump::common::ringbuffer*>(d)->pop_uint_8();
                        ...
                        if((u&0b10000000)!=0)
                            ...
                            return pump::reactor::make_imme_flow(std::forward<u_int32_t>(r));
                    });
    }
                    ...
                    return wait_remaining(std::forward<mqtt_session<_TCP_IMPL_>>(session),0,0)
                            .then([session=std::forward<mqtt_session<_TCP_IMPL_>>(session),cmd=std::get<u_int8_t>(v)](FLOW_ARG(u_int32_t)&& v)mutable{
                                ...
                                return session._inner_data_->inner_tcp.wait_packet(TIMEOUT_MS,remaining)
                                        .then([cmd,remaining](FLOW_ARG(std::variant<int,pump::common::ringbuffer*>)&& v){
                                            ____forward_flow_monostate_exception(v);
                                            auto&& d=std::get<std::variant<int,pump::common::ringbuffer*>>(v);
                                            ____throw_timeout(d);
                                            return std::make_tuple(cmd,remaining,std::get<pump::common::ringbuffer*>(d));
                                        });
                            });
                });
    }
