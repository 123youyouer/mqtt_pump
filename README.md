...
----超高并发C++服务器无锁框架（PUMP）。以及基于框架开发的业务系统（APP）（开发中）

----目前已经实现基于DPDK(f-stack,freeBSD的TCP栈)的高速网络模块

----目前已经实现基于SPDK NVME 的块存储部分

----目前的开发目标是基于框架和DPDK/SPDK底层实现支持redis协议的高速持久化的分布式缓存
        ...
        每秒插入28472452条记录（纯内存运算，数据类型为int，只是为了测试框架任务并发的速度，不涉及IO）
